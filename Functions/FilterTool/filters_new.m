function varargout = filters_new(varargin)
% FILTERS_NEW MATLAB code for filters_new.fig
%      FILTERS_NEW, by itself, creates a new FILTERS_NEW or raises the existing
%      singleton*.
%
%      H = FILTERS_NEW returns the handle to a new FILTERS_NEW or the handle to
%      the existing singleton*.
%
%      FILTERS_NEW('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in FILTERS_NEW.M with the given input arguments.
%
%      FILTERS_NEW('Property','Value',...) creates a new FILTERS_NEW or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before filters_new_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to filters_new_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help filters_new

% Last Modified by GUIDE v2.5 24-Feb-2014 19:01:16

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @filters_new_OpeningFcn, ...
                   'gui_OutputFcn',  @filters_new_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
function filters_new_OpeningFcn(hObject, eventdata, handles, varargin)
handles.output = hObject;

guidata(hObject, handles);
% UIWAIT makes filters_new wait for user response (see UIRESUME)
% uiwait(handles.figure1);

function varargout = filters_new_OutputFcn(hObject, eventdata, handles) 
